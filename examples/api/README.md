# api examples (hand-maintained per language)

> **Preview.** The `pulumiservice:api:*` resource surface is in preview. Resource
...
the supported production interface. The api surface at `pulumiservice:api:*`
exists in the same plugin and runs side-by-side with v0.

Switching an existing managed resource from a v0 token to an api token *without*
a Pulumi `alias` will register the change as a delete-and-recreate. The api
metadata does **not** ship implicit v0↔api aliases — module layout and renames
...
api resource. Look up the v0↔api mapping in
[`docs/v0-api-coverage.md`](../../docs/v0-api-coverage.md).

```typescript
import * as pulumiservice from "@pulumi/pulumiservice";

new pulumiservice.api.teams.Team("example", {
...
pulumiservice.api.teams.Team("example",
...
End-to-end tests of the `pulumiservice:api:*` resource surface. Each example
...
mishandles multi-module schemas (the `api` module under the `pulumiservice`
...
- resolve `pulumiservice:api:*` tokens against the in-process provider,
- exercise the api Check + Diff path,

…they just stop short of calling Create against the live API because
the create surface for those resources is incomplete in api metadata.
...
go test -tags api -run TestApi -v -timeout 30m ./examples
```

To exercise just one example or one lane:

```bash
go test -tags api -run "TestApi/webhooks/yaml"     -v ./examples
go test -tags api -run "TestApi/.+/typescript"     -v ./examples
```
