...
- 1 × `pulumiservice:api/teams:Team` — `catalog-owner-<suffix>` (the owner)
- 3 × `pulumiservice:Stack` — placeholder stacks the catalog points at
- 3 × `pulumiservice:api/services:Service` — `checkout-api`, `search-frontend`,
...
shape. Switching to api turns the catalog into proper Pulumi state.
