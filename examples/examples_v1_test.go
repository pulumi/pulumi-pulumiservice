...
// V1 integration tests over hand-maintained per-language examples under
// examples/v1/<name>/{yaml,typescript,python,go,csharp,java}/.
//
...
// `pulumi convert` mishandles multi-module schemas (our `v1` module under
...
//
// TestV1 iterates every (example, language) pair. The yaml lane uses the
// in-process provider directly and runs full up/preview-no-changes/refresh
// /destroy when the v1 surface supports it. Other lanes always run
// preview-only — that's enough to verify the program compiles, resolves
// v1 type tokens against the in-process provider, and constructs valid
// resource registrations.
//
// Some examples are PreviewOnlyAll because their v1 create operations
...
type v2Case struct {
	// Name is the directory under examples/v1/<name>/.
	Name string
	...
	// Used when the v1 surface is partial or blocked at create time.
	PreviewOnlyAll bool
	...
	// Dir overrides the example directory under examples/v1/. Defaults to
...
var v2Cases = []v2Case{
	...
	},
	{
		...
		Config:  stackConfig,
		FullE2E: true,
		// Stack v1 has no top-level Update endpoint — tag mutation goes
	...
	},
	{
		...
		PreviewOnlyAll: true, // Tagging a specific revision needs a definition pushed to the env (revision 1+); a freshly-created v1 Environment has no revisions, so a real Up would 404.
...
func runV2Case(t *testing.T, ex v2Case, lang string) {
	...
	src := filepath.Join(getCwd(t), "v1", dir, lang)
...
func skipLangReason(t *testing.T, lang string) string {
	...
	case "yaml":
		return ""
	case "typescript":
		if _, err := os.Stat(filepath.Join(root, "sdk", "nodejs", "bin", "v1")); err != nil {
			return "local @pulumi/pulumiservice (sdk/nodejs/bin) not built; run `make build_nodejs install_nodejs_sdk`"
		}
	case "python":
		if _, err := os.Stat(filepath.Join(root, "sdk", "python", "pulumi_pulumiservice", "v1")); err != nil {
	...
	case "go":
		if _, err := os.Stat(filepath.Join(root, "sdk", "go", "pulumiservice", "v1")); err != nil {
			return "local pulumiservice/v1 (sdk/go) not built; run `make build_go`"
		}
	case "csharp":
...
func policyGroupsConfig() map[string]string {
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"groupName":        "v1-pg-" + generateRandomFiveDigits(),
	}
}
...
func schedulesConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "v1-schedules-" + suffix,
		"stackName":        "dev-" + suffix,
		"envName":          "v1-sched-env-" + suffix,
		"scheduleCron":     "0 7 * * *",
	}
}
...
func accessTokensConfig() map[string]string {
	return map[string]string{
		...
		"tokenDescription": "v1 access-tokens example",
	}
}
...
func agentPoolsConfig() map[string]string {
	return map[string]string{
		...
		"poolDescription":  "v1 example agent pool",
	}
}
...
func stackTagsConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "v1-stack-tags-" + suffix,
		"stackName":        "dev-" + suffix,
		"tagValue":         "v1-tag-" + suffix,
	}
}
...
func environmentConfigConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "v1-envcfg-" + suffix,
		"envName":          "v1-envcfg-env-" + suffix,
	}
}
...
func stackConfigCatalogConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "v1-stack-config-" + suffix,
		"stackName":        "dev-" + suffix,
...
func taskConfig() map[string]string {
	return map[string]string{
		...
		"taskID":           "v1-task-" + generateRandomFiveDigits(),
	}
}
...
func escWebhookConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "v1-esc-hook-" + suffix,
...
func escEnvironmentScheduleConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "v1-esc-sched-" + suffix,
...
func escRevisionTagConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "v1-esc-revtag-" + suffix,
