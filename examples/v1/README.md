# v1 examples (hand-maintained per language)

> **Preview.** The `pulumiservice:v1:*` resource surface is in preview. Resource
> shape and module layout may change before GA; not yet recommended for
> production.

## Migrating from v0

The v0 surface lives at `pulumiservice:index:*` (the package root) and remains
the supported production interface. The v1 surface at `pulumiservice:v1:*`
exists in the same plugin and runs side-by-side with v0.

Switching an existing managed resource from a v0 token to a v1 token *without*
a Pulumi `alias` will register the change as a delete-and-recreate. The v1
metadata does **not** ship implicit v0↔v1 aliases — module layout and renames
may still shift before GA, so any aliases need to come from the user program.

To migrate one resource at a time, add the v0 type token as an `alias` on the
v1 resource. Look up the v0↔v1 mapping in
[`docs/v0-v1-coverage.md`](../../docs/v0-v1-coverage.md).

```typescript
import * as pulumiservice from "@pulumi/pulumiservice";

new pulumiservice.v1.teams.Team("example", {
...
pulumiservice.v1.teams.Team("example",
...
resources:
  example:
    type: pulumiservice:v1/teams:Team
    properties:
      organizationName: acme
      name: platform
      teamType: pulumi
...
ability to roll back to the v0 token.

End-to-end tests of the `pulumiservice:v1:*` resource surface. Each example
...
mishandles multi-module schemas (the `v1` module under the `pulumiservice`
...
- resolve `pulumiservice:v1:*` tokens against the in-process provider,
- exercise the v1 Check + Diff path,

…they just stop short of calling Create against the live API because
the create surface for those resources is incomplete in v1 metadata.
...
go test -tags v1 -run TestV1 -v -timeout 30m ./examples
```

To exercise just one example or one lane:

```bash
go test -tags v1 -run "TestV1/webhooks/yaml"     -v ./examples
go test -tags v1 -run "TestV1/.+/typescript"     -v ./examples
```
