# Service Catalog v1 demo

Provisions a Pulumi Cloud **service catalog** entirely from one Pulumi
program. v0 pulumiservice has no equivalent — there is no v0 resource
...
- 1 × `pulumiservice:v1/teams:Team` — `catalog-owner-<suffix>` (the owner)
- 3 × `pulumiservice:Stack` — placeholder stacks the catalog points at
- 3 × `pulumiservice:v1/services:Service` — `checkout-api`, `search-frontend`,
...
Show this snippet from a v0 program — the only way to seed the catalog
without v1:
...
shape. Switching to v1 turns the catalog into proper Pulumi state.
