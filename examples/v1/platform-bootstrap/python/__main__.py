...
ps_v1.DefaultOrganization("defaultOrg", org_name=organization_name)

ps_v1.auth.OidcIssuer(
    "githubIssuer",
...
platform_team = ps_v1.teams.Team(
    "platformTeam",
    org_name=organization_name,
...
ci_token = ps_v1.tokens.OrgToken(
    "ciToken",
    org_name=organization_name,
    name=f"ci-{suffix}",
...
runners_pool = ps_v1.agents.Pool(
    "runnersPool",
    org_name=organization_name,
...
templates = ps_v1.OrgTemplateCollection(
    "templates",
    org_name=organization_name,
...
shared_credentials = ps_v1.esc.Environment(
    "sharedCredentials",
    org_name=organization_name,
    project="shared",
    name=f"credentials-{suffix}",
)
ps_v1.esc.EnvironmentTag(
    "stableTag",
    org_name=organization_name,
...
staging_stack = ps_v1.stacks.Stack(
    "stagingStack",
    org_name=organization_name,
    project_name=f"platform-app-{suffix}",
    stack_name="staging",
)
prod_stack = ps_v1.stacks.Stack(
    "prodStack",
    org_name=organization_name,
    project_name=f"platform-app-{suffix}",
    stack_name="prod",
...
prod_deploy_settings = ps_v1.deployments.Settings(
    "prodDeploySettings",
    org_name=organization_name,
    project_name=prod_stack.project_name,
    stack_name=prod_stack.stack_name,
...
ps_v1.deployments.ScheduledDeployment(
    "prodNightlyDeploy",
    org_name=organization_name,
    project_name=prod_stack.project_name,
    stack_name=prod_stack.stack_name,
