...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		if tokenDescription == "" {
			tokenDescription = "example v1 access token"
		}

		team, err := teams.NewTeam(ctx, "team", &teams.TeamArgs{
			OrgName:     pulumi.String(organizationName),
			Name:        pulumi.String("v1-tokens-team-" + tokenSuffix),
			DisplayName: pulumi.String("v1 Tokens Team " + tokenSuffix),
			Description: pulumi.String("Owner team for the v1 access-tokens example"),
		})
		if err != nil {
			return err
		}
		...
		orgToken, err := tokens.NewOrgToken(ctx, "orgToken", &tokens.OrgTokenArgs{
			OrgName:     pulumi.String(organizationName),
			Name:        pulumi.String("v1-org-token-" + tokenSuffix),
		...
		teamToken, err := tokens.NewTeamToken(ctx, "teamToken", &tokens.TeamTokenArgs{
			OrgName:     pulumi.String(organizationName),
			TeamName:    team.Name,
			Name:        pulumi.String("v1-team-token-" + tokenSuffix),
			Description: pulumi.String(tokenDescription),
			Expires:     pulumi.Int(0),
		})
		if err != nil {
