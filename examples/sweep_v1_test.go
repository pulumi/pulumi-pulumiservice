...
// Pre-test sweeper for the v1 test org. The TestV1 suite creates real
...
//
// sweepOnce runs once per `go test` invocation, before TestV1 starts,
...
// sweepV1TestState removes leftover test resources from the configured
// Pulumi org. Best-effort: failures are logged, never fatal.
func sweepV1TestState(t *testing.T) {
	sweepOnce.Do(func() {
...
//   - prefixes: "v1-", "test-", "org-webhook-", "pulumiIssuer", etc.
//
...
func (s *sweeper) shouldSweep(name string) bool {
	for _, pre := range []string{
		"v1-", "test-", "org-webhook-", "stack-webhook-",
		"env-webhook-", "brand-new-v1-", "example-policy-group",
