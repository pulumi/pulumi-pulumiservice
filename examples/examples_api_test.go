...
// API-surface integration tests over hand-maintained per-language examples under
// examples/api/<name>/{yaml,typescript,python,go,csharp,java}/.
//
...
// `pulumi convert` mishandles multi-module schemas (our `api` module under
...
//
// TestApi iterates every (example, language) pair. The yaml lane uses the
// in-process provider directly and runs full up/preview-no-changes/refresh
// /destroy when the api surface supports it. Other lanes always run
// preview-only — that's enough to verify the program compiles, resolves
// api type tokens against the in-process provider, and constructs valid
// resource registrations.
//
// Some examples are PreviewOnlyAll because their api create operations
// are blocked by missing idField support, mutation-style API shapes, or
// non-idempotent endpoints; see the apiCases table.
...
type apiCase struct {
	// Name is the directory under examples/api/<name>/.
	Name string
	...
	// Used when the api surface is partial or blocked at create time.
	PreviewOnlyAll bool
	...
	// Dir overrides the example directory under examples/api/. Defaults to
...
var apiCases = []apiCase{
	...
	},
	{
		...
		Config:  stackConfig,
		FullE2E: true,
		// Stack api has no top-level Update endpoint — tag mutation goes
	...
	},
	{
		...
		PreviewOnlyAll: true, // Tagging a specific revision needs a definition pushed to the env (revision 1+); a freshly-created api Environment has no revisions, so a real Up would 404.
...
var apiLanguages = []string{"yaml", "typescript", "python", "go", "csharp", "java"}

func TestApi(t *testing.T) {
	sweepApiTestState(t)
	for _, ex := range apiCases {
		ex := ex
		for _, lang := range apiLanguages {
			lang := lang
			t.Run(ex.Name+"/"+lang, func(t *testing.T) {
				...
				runApiCase(t, ex, lang)
			})
		}
	}
}

// runApiCase stages the per-language example into a temp dir, applies any
// language-specific local-SDK linkage, and runs preview (or full E2E for
// the yaml lane on FullE2E cases).
func runApiCase(t *testing.T, ex apiCase, lang string) {
	dir := ex.Dir
	if dir == "" {
		dir = ex.Name
	}
	src := filepath.Join(getCwd(t), "api", dir, lang)
...
func skipLangReason(t *testing.T, lang string) string {
	...
	case "yaml":
		return ""
	case "typescript":
		if _, err := os.Stat(filepath.Join(root, "sdk", "nodejs", "bin", "api")); err != nil {
			return "local @pulumi/pulumiservice (sdk/nodejs/bin) not built; run `make build_nodejs install_nodejs_sdk`"
		}
	case "python":
		if _, err := os.Stat(filepath.Join(root, "sdk", "python", "pulumi_pulumiservice", "api")); err != nil {
	...
	case "go":
		if _, err := os.Stat(filepath.Join(root, "sdk", "go", "pulumiservice", "api")); err != nil {
			return "local pulumiservice/api (sdk/go) not built; run `make build_go`"
		}
	case "csharp":
...
func schedulesConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "api-schedules-" + suffix,
		"stackName":        "dev-" + suffix,
		"envName":          "api-sched-env-" + suffix,
		"scheduleCron":     "0 7 * * *",
	}
}
...
func accessTokensConfig() map[string]string {
	return map[string]string{
		...
		"tokenDescription": "api access-tokens example",
	}
}
...
func stackTagsConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "api-stack-tags-" + suffix,
		"stackName":        "dev-" + suffix,
		"tagValue":         "api-tag-" + suffix,
	}
}
...
func environmentConfigConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "api-envcfg-" + suffix,
		"envName":          "api-envcfg-env-" + suffix,
	}
}
...
func stackConfigCatalogConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "api-stack-config-" + suffix,
		"stackName":        "dev-" + suffix,
...
func taskConfig() map[string]string {
	return map[string]string{
		...
		"taskID":           "api-task-" + generateRandomFiveDigits(),
	}
}
...
func escWebhookConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "api-esc-hook-" + suffix,
...
func escEnvironmentScheduleConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "api-esc-sched-" + suffix,
...
func escRevisionTagConfig() map[string]string {
	...
	return map[string]string{
		"organizationName": ServiceProviderTestOrg,
		"projectName":      "api-esc-revtag-" + suffix,
