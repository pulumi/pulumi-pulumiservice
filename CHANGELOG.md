...
- `OrganizationRole.permissions` now exposes Pulumi Cloud's wire-format permission descriptor grammar verbatim, with the descriptor-type field renamed from `__type` to `discriminator` (Pulumi's Python SDK strips `__`-prefixed keys from inputs — pulumi/pulumi#22738 — so `__type` cannot be used directly; `discriminator` is also reserved against future Cloud models that may legitimately carry a domain `kind` field). Every descriptor variant Pulumi Cloud accepts — `PermissionDescriptorAllow`, `PermissionDescriptorGroup`, `PermissionDescriptorCondition`, `PermissionDescriptorCompose`, `PermissionDescriptorIfThenElse`, `PermissionDescriptorSelect`, the `PermissionExpression*` and `PermissionLiteralExpression*` boolean operators, and any future Cloud addition — passes through the provider untouched. This unblocks `pulumi import` of complex roles authored in the Pulumi Cloud UI (e.g. `PermissionDescriptorCompose`) that the previous translator's hard-coded variant allowlist rejected. The provider's only structural change to the wire shape is wrapping a top-level `PermissionDescriptorCondition` in a single-entry `PermissionDescriptorGroup` on Create/Update and reversing the wrap on Read — a workaround for a Pulumi Cloud role-detail UI 500 on bare top-level Conditions. Pre-release polish on the unreleased RBAC API.
