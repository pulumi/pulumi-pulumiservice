...
- `getPolicyPacks.policyPacks` is now a list of typed `PolicyPackSummary` objects instead of an untyped string map (`[]map[string]string` in Go, `{[key: string]: string}[]` in TypeScript, `Sequence[Mapping[str, str]]` in Python). The wire format is unchanged. The previous SDK type came from an inline object in the schema's array `items`, which schema generation flattens to a bare `object`, so it could not represent `versions` (a list of integers) or `versionTags` (a list of strings), and could not carry the new `source` and `publisher` fields. Impact by language:
  - **Go**: no working code to migrate. The old type could not deserialize `versions`. The plain `GetPolicyPacks` invoke returned an empty list with no error, and the `GetPolicyPacksOutput` invoke failed with `expected a string, got a []`. Read fields such as `pack.Name` from the new type.
  - **Python**: no working code to migrate. The old annotation made `get_policy_packs()` raise during deserialization before any user code ran. The new `PolicyPackSummary` is a `dict` subclass keyed by snake_case: use `pack.display_name` or `pack["display_name"]`. `pack["displayName"]` raises `KeyError`.
  - **TypeScript**: compile-time only. Runtime values are identical and `pack.name` keeps working. Dynamic indexing (`pack[someVariable]`) and passing the array to a `{[key: string]: string}[]` parameter no longer type-check.
  - **.NET / Java**: the only languages where the old surface worked, and only for `name` and `displayName`. Replace dictionary access with typed properties: `pack["name"]` becomes `pack.Name` in .NET and `pack.get("name")` becomes `pack.name()` in Java. `versions` and `versionTags`, previously null or empty, now carry values.

  `getPolicyPack` (singular) is unchanged apart from the new optional fields. Invoke results are not persisted in state, so there is no state migration, refresh drift, or import impact. [#1013](https://github.com/pulumi/pulumi-pulumiservice/issues/1013)
...
- Fixed `Stack.forceDestroy` forcing a replacement of the stack. The flag is only consulted at delete time, but v1.1.0 declared it `replaceOnChanges`, and providers before v1.1.0 wrote it to state only when `true` — so on the first update after upgrading a 0.x-created `Stack`, the diff `forceDestroy: <absent> → false` scheduled a replacement, which then failed because the stack already exists. Changing `forceDestroy` is now a state-only update, and state that omits the flag no longer diffs against a program that leaves it unset. The SDKs declared the same replacement client-side, so both the SDK package and the provider plugin must be on a fixed version. [#1093](https://github.com/pulumi/pulumi-pulumiservice/issues/1093)
...
- First stable release. No user-facing changes beyond 0.41.0; the embedded OpenAPI spec was refreshed and the SDKs regenerated. [#853](https://github.com/pulumi/pulumi-pulumiservice/pull/853)
...
- Added `buildAllowPermissions` helper data source for the simplest descriptor — a flat `PermissionDescriptorAllow` granting the supplied scopes globally on the matching resource type. Use it instead of hand-authoring the `{__type: "PermissionDescriptorAllow", permissions: [...]}` literal so the wire-format discriminator stays an implementation detail. Pairs with the scoped helpers below.
...
- Renamed the `OrganizationRole.permissions` discriminator from `__type` to `kind`. Superseded in 0.40.0, which restored `__type`. [#778](https://github.com/pulumi/pulumi-pulumiservice/pull/778)
