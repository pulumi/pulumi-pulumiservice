...
- Added `buildEnvironmentScopedPermissions`, `buildStackScopedPermissions`, and `buildInsightsAccountScopedPermissions` helper data sources that build an `OrganizationRole.permissions` descriptor for a single entity and a set of scopes. Each returns a `kind: "allow"` descriptor with the appropriate `on:` modifier (`{environment: <uuid>}` / `{stack: <id>}` / `{insightsAccount: <id>}`), so callers don't need to spell the modifier out by hand.
- Reshaped `OrganizationRole.permissions` to two kinds plus an optional `on:` modifier. Descriptors are `{kind: "allow", permissions: [...]}` (terminal grant) or `{kind: "group", entries: [...]}` (composition); either may carry an `on:` modifier — `{environment: <uuid>}` / `{stack: <id>}` / `{insightsAccount: <id>}` — to scope the grant to a single entity. The provider translates `on:` into the Pulumi Cloud API's `Condition(Equal(Expression<E>, Literal<E>(id)), <inner>)` wire shape internally. Replaces the older `__type`-discriminated wire format that previously leaked through the SDK. Removes the Python-only `permissionsJson` sibling output on the three `buildXScopedPermissions` helpers — Python users now consume `.permissions` directly like every other language. Pre-release polish on the unreleased RBAC API.
