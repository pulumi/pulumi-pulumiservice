...
- `Config.Configure` now falls back to `PULUMI_BACKEND_URL` when the engine hands the provider an empty `apiUrl`. The `infer.SetDefault` registered in `Annotate` only fires for *absent* fields; an explicit empty string passes through, which is what `pulumi import` persists as the default-provider's `inputs: {}`. On subsequent destroy of imported resources against non-prod Pulumi Cloud deployments (review stacks, self-hosted), this used to silently dial `api.pulumi.com` per `NewClient`'s default and 401 with `No credentials provided or are invalid` — the original `pulumi up`/`import` pair both worked because the engine persists resolved provider config from `up` correctly, but the empty-inputs round-trip from `import` exposed the gap. Symmetric with the existing `PULUMI_ACCESS_TOKEN` fallback that already covered the same path for the access token.
