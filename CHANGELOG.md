...
- Added `buildEnvironmentScopedPermissions`, `buildStackScopedPermissions`, `buildInsightsAccountScopedPermissions`, and `buildTeamScopedPermissions` helper data sources. Each returns a `PermissionDescriptorCondition(Equal(Expression<E>, Literal<E>(id)), PermissionDescriptorAllow(perms))` tree ready to assign to `OrganizationRole.permissions`, so callers don't have to hand-author the boolean expression for the common case of "grant these scopes when the request is on entity X".
