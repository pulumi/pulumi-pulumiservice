...
- Added `executorRootPath` to `DeploymentSettings.executorContext` for overriding the executor root directory (`/`). This can be useful when running with non-root users (e.g., set to `/tmp`).
