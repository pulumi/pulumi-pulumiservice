...
- `pulumiservice:v2:*` resources whose underlying API is upsert-shaped (PUT/PATCH or `create == update` operationId) now error on `pulumi up` if the resource already exists out-of-band, instead of silently overwriting. To adopt an existing resource, use the `import` ResourceOption with the resource's ID. Affects: `AuditLogExportConfiguration`, `auth.Policy`, `auth.SAML`, `DefaultOrganization`, `deployments.Settings`, `esc.EnvironmentSettings`, `insights.ScheduledScanSettings`, `integrations.AzureDevOpsIntegration`, `integrations.BitBucketIntegration`, `integrations.GitHubEnterpriseIntegration`, `integrations.GitHubIntegration`, `integrations.GitLabIntegration`, `PolicyIssue`, `stacks.Config`, `teams.Role` (all under `pulumiservice.v2.*`).
- v0 → v2 migration is opt-in: `pulumiservice:index:*` resources continue to work unchanged, so there is no forced migration. To move a resource to v2, change its type to the v2 form and add an explicit `aliases: [{ type: "pulumiservice:index:<Name>" }]` ResourceOption on the v2 declaration so state rebinds in place. v2 resources are organized into modules, so most tokens change shape (e.g. `pulumiservice.Stack` → `pulumiservice.v2.stacks.Stack`, `pulumiservice.Team` → `pulumiservice.v2.teams.Team`, `pulumiservice.DeploymentSettings` → `pulumiservice.v2.deployments.Settings`, `pulumiservice.OidcIssuer` → `pulumiservice.v2.auth.OidcIssuer`, `pulumiservice.InsightsAccount` → `pulumiservice.v2.insights.Account`). The schedule resources unify: `DeploymentSchedule`, `DriftSchedule`, and `TtlSchedule` all migrate to `pulumiservice.v2.deployments.ScheduledDeployment` — pick the schedule kind (`request.operation`) appropriate to your use. `EnvironmentRotationSchedule` migrates to `pulumiservice.v2.esc.EnvironmentSchedule`.

  The following migrations require additional decisions:
  - `pulumiservice.Webhook` is split into three v2 resources by scope. Pick the matching one and alias it to `pulumiservice:index:Webhook`: `pulumiservice.v2.OrganizationWebhook` (org-scope), `pulumiservice.v2.stacks.Webhook` (stack-scope), or `pulumiservice.v2.esc.Webhook` (ESC environment-scope).
  - `pulumiservice.StackTags` (manages multiple tags as one resource) doesn't have a v2 counterpart with matching semantics. Migrate to one `pulumiservice.v2.stacks.Tag` per tag.

  `TeamEnvironmentPermission` and `TeamStackPermission` stay at `pulumiservice:index:*`. They're superseded by the role-based RBAC shipped in v2 — use `OrganizationRole` + `TeamRoleAssignment` with `buildStackScopedPermissions` / `buildEnvironmentScopedPermissions` to assign team permissions on individual stacks or environments. The v0 resources remain available for backwards compatibility but new code should use roles.

### Improvements
- Renamed the `pulumiservice:v2/preview:*` module to `pulumiservice:v2/escPreview:*` for the four `preview/environments`-derived ESC resources (`Environment`, `EnvironmentTag`, `RevisionTag`, `Webhook`). Existing stacks rebind via Pulumi resource aliases; user code should update imports from `pulumiservice.v2.preview.*` (or per-language equivalent) to `pulumiservice.v2.escPreview.*`.
- `pulumiservice:v2:*` resources: path parameters are now input-only and recovered from the resource ID, removing the redundant echo in resource state outputs. Program-owned identity fields no longer occupy the cloud-owned output namespace.
- Added the `pulumiservice:v2:*` resource namespace **(Preview)**, an OpenAPI-driven layer covering Stacks, Teams, Tokens, Webhooks, ESC Environments, Deployment Settings, OIDC Issuers, Roles, Policy Groups/Packs, VCS Integrations, and related Pulumi Cloud resources. Existing `pulumiservice:index:*` resources continue to work unchanged; v2 resources are accessed under `pulumiservice.v2.*` in user code. Resource shape and module layout may change before GA.
