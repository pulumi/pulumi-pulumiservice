...
- `OrganizationRole.permissions` discriminator field renamed from `kind` to `discriminator`. Programs that adopted the unreleased `kind:` discriminator (introduced in [#778](https://github.com/pulumi/pulumi-pulumiservice/pull/778)) need to rename it to `discriminator:` at every level of the descriptor tree. The discriminator *values* (`PermissionDescriptorAllow`, `PermissionDescriptorCompose`, etc.) are unchanged. Migration:
  ```yaml
  permissions:
  ...
  +   discriminator: PermissionDescriptorAllow
      permissions:
        - stack:read
  ```
  The rename avoids collisions with future Pulumi Cloud models that may carry a domain `kind` field (`apitype` already has `TeamKind`, `PolicyIssueKind`, `ScheduledActionKind`, etc.). The new `buildAllowPermissions` / `build*ScopedPermissions` helper data sources hide the field name entirely — prefer those over hand-authored descriptors. The RBAC feature is unreleased so impact is limited to early adopters.
...
- Added `buildAllowPermissions` helper data source for the simplest descriptor — a flat `PermissionDescriptorAllow` granting the supplied scopes globally on the matching resource type. Use it instead of hand-authoring the `{discriminator: "PermissionDescriptorAllow", permissions: [...]}` literal so the SDK boundary's discriminator field name stays an implementation detail. Pairs with the scoped helpers below.
