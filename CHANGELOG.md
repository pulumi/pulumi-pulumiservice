...
- `Config.Configure` now falls back to `PULUMI_BACKEND_URL` when the engine hands the provider an empty `apiUrl`. The `infer.SetDefault` registered in `Annotate` only fires for *absent* fields; an explicit empty string passes through, which happened on `pulumi destroy` of stacks whose default-provider config was persisted without an `apiUrl` override. Without this fallback, destroys against non-prod Pulumi Cloud deployments (review stacks, self-hosted) silently dialed `api.pulumi.com` and 401'd with `No credentials provided or are invalid`, even though the same token authenticated against the configured backend during `pulumi up`/`import` moments earlier in the same shell. Symmetric with the existing `PULUMI_ACCESS_TOKEN` fallback.
