...
- `pulumiservice:v2:*` resources whose underlying API is upsert-shaped (PUT/PATCH or `create == update` operationId) now error on `pulumi up` if the resource already exists out-of-band, instead of silently overwriting. To adopt an existing resource, use the `import` ResourceOption with the resource's ID. Affects: `AuditLogExportConfiguration`, `AuthPolicy`, `AzureDevOpsIntegration`, `BitBucketIntegration`, `DefaultOrganization`, `DeploymentSettings`, `EnvironmentSettings`, `GitHubEnterpriseIntegration`, `GitHubIntegration`, `GitLabIntegration`, `PolicyIssue`, `SAMLOrganization`, `ScheduledScanSettings`, `StackConfig`, `TeamRole`.
- v0 → v2 migration. Update type tokens (e.g., `pulumiservice.Stack` → `pulumiservice.v2.Stack`) and `pulumi up`. State auto-rebinds via aliases for every v0 resource that has a v2 counterpart (no manual `aliases` ResourceOption needed). The schedule resources unify: `DeploymentSchedule`, `DriftSchedule`, and `TtlSchedule` all migrate to `pulumiservice.v2.ScheduledDeployment` — pick the v2 schedule kind appropriate to your use, then `pulumi up`. `EnvironmentRotationSchedule` migrates to `pulumiservice.v2.EnvironmentSchedule`.

  The following migrations require manual decisions:
  - `pulumiservice.Webhook` is split into three v2 resources by scope. Pick the matching one and add an explicit alias: `pulumiservice.v2.OrganizationWebhook` (org-scope), `pulumiservice.v2.StackWebhook` (stack-scope), or `pulumiservice.v2.Webhook_esc_environments` (ESC environment-scope), with `aliases: [{ type: "pulumiservice:index:Webhook" }]`.
  - `pulumiservice.StackTags` (manages multiple tags as one resource) doesn't have a v2 counterpart with matching semantics. Migrate to one `pulumiservice.v2.StackTag` per tag.
