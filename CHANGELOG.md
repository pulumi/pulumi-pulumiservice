...
- Added `buildEnvironmentScopedPermissions`, `buildStackScopedPermissions`, and `buildInsightsAccountScopedPermissions` helper data sources that build an `OrganizationRole.permissions` descriptor for a single entity and a set of scopes, so callers no longer need to hand-roll the underlying `group` / `condition` / `literalEnvironment` (or `literalStack` / `literalInsightsAccount`) tree by hand.
- Renamed the `OrganizationRole.permissions` discriminator field from `__type` to `kind` and re-keyed the variants to short forms (`allow`, `group`, `condition`, `equal`, `expressionEnvironment`, `expressionStack`, `expressionInsightsAccount`, `literalEnvironment`, `literalStack`, `literalInsightsAccount`). The provider translates to the Pulumi Cloud API's `__type` wire format internally. Removes the Python-only `permissionsJson` sibling output on the three `buildXScopedPermissions` helpers — Python users can now consume `.permissions` directly like every other language. Pre-release polish on the unreleased RBAC API.
