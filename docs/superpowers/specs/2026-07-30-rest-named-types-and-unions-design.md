...
- **Metadata**: a new top-level `types` section in metadata.json overrides emission per
  component schema (added during implementation; the yaml example sweep surfaced three
  spec-vs-wire divergences that need hand curation):
  - `scalarShorthand: "string"` → emit `oneOf: [string, <named type>]` where the service's
    deserializer accepts a scalar shorthand the model doesn't express (`DockerImageRequest`;
    verified in pulumi-service `apitype/deployments_executor_test.go`).
  - `any: true` → degrade to `pulumi.json#/Any` where the spec model doesn't match the wire
    (`AddServiceItem` is response-shaped but used in the create request) or where upstream
    codegen breaks (`SecretValue`: a secret property typed as a 2-member union hits a
    pulumi-language-dotnet bug — the generated secret setter uses the backing
    `InputUnion` field type in `Output.Tuple`, which does not compile; report upstream).
  - `optional: [..]` → strip spec-required fields the service defaults
    (`ServiceProperty.order/type`).
  The scaffolder round-trips the section untouched. `fields` metadata continues to apply to
  top-level resource properties only.
...
- **Marker subtypes render as the base union** (added 2026-07-31 after the deep-RBAC
  audit): a `$ref` to a schema whose allOf chain reaches a discriminated ancestor and that
  adds no properties of its own (e.g. `PermissionBooleanExpression` over
  `PermissionExpression`) emits the ancestor's `oneOf` instead of a closed `{tag}`-only
  named type. Without this, `Condition.condition` rejected every expression variant, so
  env/stack/team-scoped permission trees — the production RBAC shape — failed typechecking.
  Opens the full expression algebra (And/Or/Not/Equal/HasTag/Select); 29 union sites total.
...
### Compatibility notes (learned in CI)

- Go/C#/Java programs that passed raw maps into previously-`Any` nested inputs break when
  the slot gains a concrete type; union-typed slots stay untyped and keep accepting maps.
  TS/Python/YAML are structural and unaffected. The api namespace is preview; examples were
  rewritten to the typed forms.
- `pulumi package gen-sdk` output ordering is platform-dependent (macOS vs linux collation
  of `__all__`/member order). Committed SDKs must be generated on linux (docker
  `pulumi/pulumi` image) or CI's digest check fails.
- CI generates SDKs with the mise-provided pulumi CLI resolved from go.mod's pkg/v3 pin
  (`PULUMI_VERSION_MISE`), currently 3.255.0.
