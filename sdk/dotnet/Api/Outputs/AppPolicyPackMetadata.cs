// *** WARNING: this file was generated by pulumi-language-dotnet. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.PulumiService.Api.Outputs
{

    /// <summary>
    /// PolicyPackMetadata is the metadata of a Policy Pack.
    /// </summary>
    [OutputType]
    public sealed class AppPolicyPackMetadata
    {
        /// <summary>
        /// The configuration that is to be passed to the Policy Pack. This
        /// map ties Policies with their configuration.
        /// </summary>
        public readonly ImmutableDictionary<string, ImmutableDictionary<string, object>>? Config;
        /// <summary>
        /// The display name
        /// </summary>
        public readonly string DisplayName;
        /// <summary>
        /// References to ESC environments to use for this policy pack.
        /// </summary>
        public readonly ImmutableArray<string> Environments;
        /// <summary>
        /// The name
        /// </summary>
        public readonly string Name;
        /// <summary>
        /// The version number
        /// </summary>
        public readonly int Version;
        /// <summary>
        /// The version tag
        /// </summary>
        public readonly string VersionTag;

        [OutputConstructor]
        private AppPolicyPackMetadata(
            ImmutableDictionary<string, ImmutableDictionary<string, object>>? config,

            string displayName,

            ImmutableArray<string> environments,

            string name,

            int version,

            string versionTag)
        {
            Config = config;
            DisplayName = displayName;
            Environments = environments;
            Name = name;
            Version = version;
            VersionTag = versionTag;
        }
    }
}
