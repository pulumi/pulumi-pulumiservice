// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.pulumiservice.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;

@CustomType
public final class GetEnvironmentResult {
    /**
     * @return The environment&#39;s UUID. Use this as the `identity` value when pinning a custom RBAC role to this environment via a `PermissionLiteralExpressionEnvironment` expression.
     * 
     */
    private String environmentId;
    /**
     * @return The environment name.
     * 
     */
    private String name;
    /**
     * @return The Pulumi Cloud organization that owns the environment.
     * 
     */
    private String organizationName;
    /**
     * @return The ESC project the environment lives in.
     * 
     */
    private String projectName;

    private GetEnvironmentResult() {}
    /**
     * @return The environment&#39;s UUID. Use this as the `identity` value when pinning a custom RBAC role to this environment via a `PermissionLiteralExpressionEnvironment` expression.
     * 
     */
    public String environmentId() {
        return this.environmentId;
    }
    /**
     * @return The environment name.
     * 
     */
    public String name() {
        return this.name;
    }
    /**
     * @return The Pulumi Cloud organization that owns the environment.
     * 
     */
    public String organizationName() {
        return this.organizationName;
    }
    /**
     * @return The ESC project the environment lives in.
     * 
     */
    public String projectName() {
        return this.projectName;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GetEnvironmentResult defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private String environmentId;
        private String name;
        private String organizationName;
        private String projectName;
        public Builder() {}
        public Builder(GetEnvironmentResult defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.environmentId = defaults.environmentId;
    	      this.name = defaults.name;
    	      this.organizationName = defaults.organizationName;
    	      this.projectName = defaults.projectName;
        }

        @CustomType.Setter
        public Builder environmentId(String environmentId) {
            if (environmentId == null) {
              throw new MissingRequiredPropertyException("GetEnvironmentResult", "environmentId");
            }
            this.environmentId = environmentId;
            return this;
        }
        @CustomType.Setter
        public Builder name(String name) {
            if (name == null) {
              throw new MissingRequiredPropertyException("GetEnvironmentResult", "name");
            }
            this.name = name;
            return this;
        }
        @CustomType.Setter
        public Builder organizationName(String organizationName) {
            if (organizationName == null) {
              throw new MissingRequiredPropertyException("GetEnvironmentResult", "organizationName");
            }
            this.organizationName = organizationName;
            return this;
        }
        @CustomType.Setter
        public Builder projectName(String projectName) {
            if (projectName == null) {
              throw new MissingRequiredPropertyException("GetEnvironmentResult", "projectName");
            }
            this.projectName = projectName;
            return this;
        }
        public GetEnvironmentResult build() {
            final var _resultValue = new GetEnvironmentResult();
            _resultValue.environmentId = environmentId;
            _resultValue.name = name;
            _resultValue.organizationName = organizationName;
            _resultValue.projectName = projectName;
            return _resultValue;
        }
    }
}
