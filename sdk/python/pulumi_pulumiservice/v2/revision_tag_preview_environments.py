# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities

__all__ = ['RevisionTag_preview_environmentsArgs', 'RevisionTag_preview_environments']

@pulumi.input_type
class RevisionTag_preview_environmentsArgs:
    def __init__(__self__, *,
                 env_name: pulumi.Input[_builtins.str],
                 org_name: pulumi.Input[_builtins.str],
                 tag_name: pulumi.Input[_builtins.str],
                 revision: Optional[pulumi.Input[_builtins.int]] = None):
        """
        The set of arguments for constructing a RevisionTag_preview_environments resource.

        :param pulumi.Input[_builtins.str] env_name: The environment name
        :param pulumi.Input[_builtins.str] org_name: The organization name
        :param pulumi.Input[_builtins.str] tag_name: The revision tag name
        :param pulumi.Input[_builtins.int] revision: The revision number to associate with the tag.
        """
        pulumi.set(__self__, "env_name", env_name)
        pulumi.set(__self__, "org_name", org_name)
        pulumi.set(__self__, "tag_name", tag_name)
        if revision is not None:
            pulumi.set(__self__, "revision", revision)

    @_builtins.property
    @pulumi.getter(name="envName")
    def env_name(self) -> pulumi.Input[_builtins.str]:
        """
        The environment name
        """
        return pulumi.get(self, "env_name")

    @env_name.setter
    def env_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "env_name", value)

    @_builtins.property
    @pulumi.getter(name="orgName")
    def org_name(self) -> pulumi.Input[_builtins.str]:
        """
        The organization name
        """
        return pulumi.get(self, "org_name")

    @org_name.setter
    def org_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "org_name", value)

    @_builtins.property
    @pulumi.getter(name="tagName")
    def tag_name(self) -> pulumi.Input[_builtins.str]:
        """
        The revision tag name
        """
        return pulumi.get(self, "tag_name")

    @tag_name.setter
    def tag_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "tag_name", value)

    @_builtins.property
    @pulumi.getter
    def revision(self) -> Optional[pulumi.Input[_builtins.int]]:
        """
        The revision number to associate with the tag.
        """
        return pulumi.get(self, "revision")

    @revision.setter
    def revision(self, value: Optional[pulumi.Input[_builtins.int]]):
        pulumi.set(self, "revision", value)


@pulumi.type_token("pulumiservice:v2:RevisionTag_preview_environments")
class RevisionTag_preview_environments(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 env_name: Optional[pulumi.Input[_builtins.str]] = None,
                 org_name: Optional[pulumi.Input[_builtins.str]] = None,
                 revision: Optional[pulumi.Input[_builtins.int]] = None,
                 tag_name: Optional[pulumi.Input[_builtins.str]] = None,
                 __props__=None):
        """
        Updates an existing revision tag for a Pulumi ESC environment to point to a different revision number. The tag is identified by its name in the URL path. The request body specifies the new revision number. This allows advancing or rolling back a named reference (e.g., moving the 'prod' tag to a newer or older revision). Returns 204 on success with no response body.


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] env_name: The environment name
        :param pulumi.Input[_builtins.str] org_name: The organization name
        :param pulumi.Input[_builtins.int] revision: The revision number to associate with the tag.
        :param pulumi.Input[_builtins.str] tag_name: The revision tag name
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: RevisionTag_preview_environmentsArgs,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        Updates an existing revision tag for a Pulumi ESC environment to point to a different revision number. The tag is identified by its name in the URL path. The request body specifies the new revision number. This allows advancing or rolling back a named reference (e.g., moving the 'prod' tag to a newer or older revision). Returns 204 on success with no response body.


        :param str resource_name: The name of the resource.
        :param RevisionTag_preview_environmentsArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(RevisionTag_preview_environmentsArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 env_name: Optional[pulumi.Input[_builtins.str]] = None,
                 org_name: Optional[pulumi.Input[_builtins.str]] = None,
                 revision: Optional[pulumi.Input[_builtins.int]] = None,
                 tag_name: Optional[pulumi.Input[_builtins.str]] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = RevisionTag_preview_environmentsArgs.__new__(RevisionTag_preview_environmentsArgs)

            if env_name is None and not opts.urn:
                raise TypeError("Missing required property 'env_name'")
            __props__.__dict__["env_name"] = env_name
            if org_name is None and not opts.urn:
                raise TypeError("Missing required property 'org_name'")
            __props__.__dict__["org_name"] = org_name
            __props__.__dict__["revision"] = revision
            if tag_name is None and not opts.urn:
                raise TypeError("Missing required property 'tag_name'")
            __props__.__dict__["tag_name"] = tag_name
            __props__.__dict__["created"] = None
            __props__.__dict__["editor_login"] = None
            __props__.__dict__["editor_name"] = None
            __props__.__dict__["modified"] = None
            __props__.__dict__["name"] = None
        super(RevisionTag_preview_environments, __self__).__init__(
            'pulumiservice:v2:RevisionTag_preview_environments',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None) -> 'RevisionTag_preview_environments':
        """
        Get an existing RevisionTag_preview_environments resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = RevisionTag_preview_environmentsArgs.__new__(RevisionTag_preview_environmentsArgs)

        __props__.__dict__["created"] = None
        __props__.__dict__["editor_login"] = None
        __props__.__dict__["editor_name"] = None
        __props__.__dict__["modified"] = None
        __props__.__dict__["name"] = None
        __props__.__dict__["revision"] = None
        return RevisionTag_preview_environments(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def created(self) -> pulumi.Output[_builtins.str]:
        """
        The timestamp when the tag was created.
        """
        return pulumi.get(self, "created")

    @_builtins.property
    @pulumi.getter(name="editorLogin")
    def editor_login(self) -> pulumi.Output[Optional[_builtins.str]]:
        """
        The login name of the user who last edited the tag.
        """
        return pulumi.get(self, "editor_login")

    @_builtins.property
    @pulumi.getter(name="editorName")
    def editor_name(self) -> pulumi.Output[Optional[_builtins.str]]:
        """
        The display name of the user who last edited the tag.
        """
        return pulumi.get(self, "editor_name")

    @_builtins.property
    @pulumi.getter
    def modified(self) -> pulumi.Output[_builtins.str]:
        """
        The timestamp when the tag was last modified.
        """
        return pulumi.get(self, "modified")

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Output[_builtins.str]:
        """
        The name of the tag.
        """
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter
    def revision(self) -> pulumi.Output[_builtins.int]:
        """
        The revision number this tag points to.
        """
        return pulumi.get(self, "revision")

