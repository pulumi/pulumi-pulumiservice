...
func (r *typeRegistry) typeSpec(nm map[string]any) schema.TypeSpec {
	if ref, ok := nm["$ref"].(string); ok {
		...
		if tagProp, mapping := discriminatorOf(target); tagProp != "" {
			...
			case 1:
				for tag, variantRef := range mapping {
					return r.refTo(r.emitVariant(refSchemaName(variantRef), tagProp, tag))
				}
			default:
...
func scalarTypeSpec(node map[string]any) schema.TypeSpec {
	...
	case "array":
		...
		if items != nil {
			// Named array aliases resolve structurally; the item type may
			// itself be a $ref, which degrades to Any here (rare).
...
func (r *typeRegistry) emit(name, tagProp, tag string) string {
	...
	if tagProp != "" {
		...
		ps.Description += fmt.Sprintf("Expected value is '%s'.", tag)
		pprops[tagProp] = ps
		reqSet[tagProp] = true
	}
