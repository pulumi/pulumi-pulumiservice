...
func circleShape() map[string]any {
	return map[string]any{tagKind: tagCircle, radiusProp: 3, "label": "a"}
}
...
func TestValidateUnknownTagSuggests(t *testing.T) {
	failures := validateWidget(t, unionSchemas(), nil, map[string]any{
		shapeProp: map[string]any{tagKind: typoTag, radiusProp: 3},
	})
	if len(failures) != 1 {
...
func TestValidateMissingTag(t *testing.T) {
	failures := validateWidget(t, unionSchemas(), nil, map[string]any{
		shapeProp: map[string]any{radiusProp: 3},
	})
...
func TestValidateSkipsComputed(t *testing.T) {
	...
	failures = validateWidget(t, unionSchemas(), nil, map[string]any{
		shapeProp: map[string]any{tagKind: property.New(property.Computed), radiusProp: 3},
	})
	if len(failures) != 0 {
...
func TestValidateSecretsAreWalked(t *testing.T) {
	failures := validateWidget(t, unionSchemas(), nil, map[string]any{
		shapeProp: pv(map[string]any{tagKind: typoTag, radiusProp: 3}).WithSecret(true),
	})
	if len(failures) != 1 {
...
func TestValidateDefiniteVariantTag(t *testing.T) {
	...
	failures := validateWidget(t, schemas, nil, map[string]any{
		oneProp: map[string]any{tagKind: tagSquare, radiusProp: 3},
	})
...
func TestCheckSurfacesValidationFailures(t *testing.T) {
	...
	resp, err := r.Check(t.Context(), p.CheckRequest{
		Inputs: property.NewMap(map[string]property.Value{
			shapeProp: pv(map[string]any{tagKind: typoTag, radiusProp: 3}),
		}),
	})
	if err != nil {
