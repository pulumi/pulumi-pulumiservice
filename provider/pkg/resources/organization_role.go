...
func (c *OrganizationRoleCore) Annotate(a infer.Annotator) {
	...
	a.Describe(
		&c.Permissions,
		"The role's permission descriptor tree. Two shapes are accepted, "+
			"and may be mixed at any node boundary.\n\n"+
			"**Structured shape** — `{kind: \"allow\", permissions: [\"<scope>\", ...]}` "+
			"to grant scopes, or `{kind: \"group\", entries: [...]}` to compose "+
			"multiple grants. Either may carry an optional `on:` modifier — a "+
			"single-key map `{environment: <uuid>}` / `{stack: <id>}` / "+
			"`{insightsAccount: <id>}` / `{team: <id>}` — to scope the descriptor "+
			"to one entity. For the common case prefer the "+
			"`buildEnvironmentScopedPermissions` / `buildStackScopedPermissions` / "+
			"`buildInsightsAccountScopedPermissions` helpers, which build the "+
			"`on:`-modified Allow for you.\n\n"+
			"**Pass-through shape** — for richer Cloud-side descriptors "+
			"(`Compose`, `Condition` with non-Equal booleans, `IfThenElse`, "+
			"`Select`, future types), use the PascalCase wire type name as the "+
			"kind value: `{kind: \"PermissionDescriptorCompose\", "+
			"permissionDescriptors: [...]}`. The descriptor and any nested "+
			"`PermissionExpression*` / `PermissionLiteralExpression*` nodes use "+
			"`kind` as their discriminator (the provider renames it to `__type` "+
			"at the wire boundary, which keeps Python's RPC deserializer "+
			"compatible). The full Cloud-side schema lives in the "+
			"`pulumi-service` repository at "+
			"`specification/src/main/java/com/pulumi/model/PermissionDescriptor.java` "+
			"and its sibling `Permission*.java` files.",
	)
}
