...
func TestInsightsAccount_Call_TriggerScan(t *testing.T) {
	...
	req := &pulumirpc.CallRequest{
		...
		Args: &structpb.Struct{
			Fields: map[string]*structpb.Value{
				"__self__": {Kind: &structpb.Value_StructValue{StructValue: selfRef}},
			},
		},
	}
	...
	fields := resp.Return.GetFields()
	assert.Equal(t, "test-scan-123", fields["scanId"].GetStringValue())
	assert.Equal(t, "running", fields["status"].GetStringValue())
	assert.Equal(t, "2025-11-12T15:30:00Z", fields["timestamp"].GetStringValue())
}

func TestInsightsAccount_Call_GetStatus(t *testing.T) {
	mockedClient := buildInsightsAccountClientMock(nil, nil, nil, nil)
	...
	// Verify response contains expected fields
	fields := resp.Return.GetFields()
	assert.Equal(t, "test-org/test-account", fields["accountId"].GetStringValue())
	assert.Equal(t, "test-account", fields["accountName"].GetStringValue())
	assert.Equal(t, "succeeded", fields["status"].GetStringValue())
	assert.Equal(t, "scan-456", fields["lastScanId"].GetStringValue())
	assert.Equal(t, "2025-11-12T14:00:00Z", fields["lastScanTime"].GetStringValue())
	assert.Equal(t, "2025-11-13T02:00:00Z", fields["nextScanTime"].GetStringValue())
	assert.Equal(t, float64(42), fields["resourceCount"].GetNumberValue())
}

func TestInsightsAccount_Call_UnknownMethod(t *testing.T) {
	mockedClient := buildInsightsAccountClientMock(nil, nil, nil, nil)
...
func TestInsightsAccount_Call_MissingSelfArg(t *testing.T) {
	mockedClient := buildInsightsAccountClientMock(nil, nil, nil, nil)
	...
	assert.Error(t, err)
	assert.Nil(t, resp)
	assert.Contains(t, err.Error(), "missing required __self__")
}

func TestInsightsAccount_Call_InvalidResourceID(t *testing.T) {
	mockedClient := buildInsightsAccountClientMock(nil, nil, nil, nil)
...
func TestInsightsAccount_Call_TriggerScan_Preview(t *testing.T) {
	mockedClient := buildInsightsAccountClientMock(nil, nil, nil, nil)
	...
	// Verify response contains computed values (indicated by special marker)
	fields := resp.Return.GetFields()

	// Computed values are serialized as the string "04da6b54-80e4-46f7-96ec-b56ff0331ba9" (the computed marker)
	// This tells Pulumi to display them as [computed] in previews
	scanIdField := fields["scanId"]
	require.NotNil(t, scanIdField, "scanId should be present")
	assert.Equal(t, "04da6b54-80e4-46f7-96ec-b56ff0331ba9", scanIdField.GetStringValue(), "scanId should have computed marker during preview")

	statusField := fields["status"]
	require.NotNil(t, statusField, "status should be present")
	assert.Equal(t, "04da6b54-80e4-46f7-96ec-b56ff0331ba9", statusField.GetStringValue(), "status should have computed marker during preview")

	timestampField := fields["timestamp"]
	require.NotNil(t, timestampField, "timestamp should be present")
	assert.Equal(t, "04da6b54-80e4-46f7-96ec-b56ff0331ba9", timestampField.GetStringValue(), "timestamp should have computed marker during preview")
}

func TestInsightsAccount_Call_GetStatus_Preview(t *testing.T) {
	mockedClient := buildInsightsAccountClientMock(nil, nil, nil, nil)
	...
	// Computed values are serialized as the string "04da6b54-80e4-46f7-96ec-b56ff0331ba9" (the computed marker)
	// This tells Pulumi to display them as [computed] in previews
	computedMarker := "04da6b54-80e4-46f7-96ec-b56ff0331ba9"

	statusField := fields["status"]
	require.NotNil(t, statusField, "status should be present")
	assert.Equal(t, computedMarker, statusField.GetStringValue(), "status should have computed marker during preview")

	accountIdField := fields["accountId"]
	require.NotNil(t, accountIdField, "accountId should be present")
	assert.Equal(t, computedMarker, accountIdField.GetStringValue(), "accountId should have computed marker during preview")

	accountNameField := fields["accountName"]
	require.NotNil(t, accountNameField, "accountName should be present")
	assert.Equal(t, computedMarker, accountNameField.GetStringValue(), "accountName should have computed marker during preview")

	lastScanIdField := fields["lastScanId"]
	require.NotNil(t, lastScanIdField, "lastScanId should be present")
	assert.Equal(t, computedMarker, lastScanIdField.GetStringValue(), "lastScanId should have computed marker during preview")

	lastScanTimeField := fields["lastScanTime"]
	require.NotNil(t, lastScanTimeField, "lastScanTime should be present")
	assert.Equal(t, computedMarker, lastScanTimeField.GetStringValue(), "lastScanTime should have computed marker during preview")

	nextScanTimeField := fields["nextScanTime"]
	require.NotNil(t, nextScanTimeField, "nextScanTime should be present")
	assert.Equal(t, computedMarker, nextScanTimeField.GetStringValue(), "nextScanTime should have computed marker during preview")

	resourceCountField := fields["resourceCount"]
	require.NotNil(t, resourceCountField, "resourceCount should be present")
	// resourceCount might use output marker (3eeb2bf0-c639-47a8-9e75-3b44932eb421) for numbers
	rcMarker := resourceCountField.GetStringValue()
	assert.NotEmpty(t, rcMarker, "resourceCount should have a marker during preview")
	assert.Contains(t, []string{computedMarker, "3eeb2bf0-c639-47a8-9e75-3b44932eb421"}, rcMarker, "resourceCount should have computed or output marker")
}
