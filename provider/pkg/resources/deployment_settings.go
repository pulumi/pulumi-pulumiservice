...
// currentStateCipherSettings are the latest outputs/properties of the resource, containing ciphertext strings of secret
// values isInput is a flag that selects whether to generating an input PropertyMap that contains plaintext (true) or an
// output PropertyMap that contains ciphertext (false)
func (ds *PulumiServiceDeploymentSettingsInput) ToPropertyMap(
	...
	// Twin value secrets are values whose plaintext cannot be retrieved from the API, thus forcing the development of
	// this fairly complex system When plaintextInputSettings is passed in, but currentStateCipherSettings is not, that
	// means the resource is being created or updated
	createMode := plaintextInputSettings != nil && currentStateCipherSettings == nil
	// When both plaintextInputSettings and currentStateCipherSettings are passed in, that means an existing resource is
	// being refreshed, and it's necessary to merge values In case we are merging, but some of the properties don't
	// previously exist, we will merge with empty value, setting plaintext to be empty string
					...
					if ds.SourceContext.Git.GitAuth.SSHAuth.SSHPrivateKey.Value != "" {
						if mergeMode {
							...
							util.ImportSecretValue(
								sshAuthPropertyMap, "sshPrivateKey",
								ds.SourceContext.Git.GitAuth.SSHAuth.SSHPrivateKey, isInput)
						}
					}
				...
				if ds.SourceContext.Git.GitAuth.BasicAuth != nil {
					...
					if ds.SourceContext.Git.GitAuth.BasicAuth.Password.Value != "" {
						if mergeMode {
							...
							util.ImportSecretValue(
								basicAuthPropertyMap, "password",
								ds.SourceContext.Git.GitAuth.BasicAuth.Password, isInput)
						}
					}
